...
```
POST {CRM}/webhooks/enso/journey-callback
...
2. **New public callback endpoint** `@Controller()` → `@Post('webhooks/enso/journey-callback')`.
   - **⚠ Must live OUTSIDE `/rest/*`:** that namespace is owned by the authenticated REST-API catch-all (`rest-api-core`, `JwtAuthGuard`), which intercepts any `/rest/...` path and rejects it with "Missing authentication token" regardless of our guards (learned the hard way — the first cut at `rest/enso/marketing/...` 403'd). Public webhooks follow the root `@Controller()` + `webhooks/*` convention (billing → `webhooks/stripe`, messaging → `webhooks/messaging/ses`).
   - **New pattern flag:** every existing enso controller (e.g. `ChatwootController`) is `JwtAuthGuard`-protected. This one is **machine-to-machine with no JWT** — public via `PublicEndpointGuard`, guarded by a **constant-time shared-secret header** (`DITTOFEED_CALLBACK_SECRET`), and takes `workspaceId` **in the body** (no auth context to derive it from). Reject on bad secret / unknown workspace / unknown person.
...
4. **Marketing-journey visibility in the CRM (section above).**
   - **Tier A — 🟢 LIVE & VERIFIED 2026-06-13.** Code in `packages/twenty-server/src/modules/enso/marketing-sync/`: `dtos/journey-callback.input.ts`, `services/marketing-journey-callback.service.ts` (upsert `marketingEnrollment` + green-sentence timeline on person *and* source opportunity), `controllers/marketing.controller.ts` (`POST /webhooks/enso/journey-callback`, public via `PublicEndpointGuard`, shared-secret `x-enso-marketing-secret`), `marketing-callback.module.ts` (server-only). Object **provisioned** (`marketingEnrollment` `b2678cc5-…`, 9 fields), `DITTOFEED_CALLBACK_SECRET` **set** on twenty-server, callback **verified** end-to-end (synthetic POST → enrollment row + timeline). **Remaining:** wire per-step Webhook-channel nodes into the journey (step 5).
